...
void DMAC_CH1_2_Handler(void)
{
	...
	case UGT_E_SS_WAIT_M:
		MDS_vWait(UG_sSysStateErr.sSysTime.ReStartTime);
		break;
	default:
		break;
	}
...
void SysTick_Handler(void)
{
	...
	case TASK2:
	{
		...
		{
			if (UG_sSystemControllers.sSPD.s32Fbk > UPDS_5STAGE_SPD_H)
				UG_sVoltageAll.sSvgen.u8PWM5Stage = 1;
			else if (UG_sSystemControllers.sSPD.s32Fbk < UPDS_7STAGE_SPD_H)
	...
	case TASK1:
	{
		if ((UG_sSysStateErr.sSystemState != UGT_E_SS_WAIT_M) && (UG_sSysStateErr.sSystemState >= UGT_E_SS_CLOSEDLOOP_J))
		{
			...
			case UGT_E_CM_TORQUE:
				UG_sSystemControllers.sCommandAll.s32IqCommand = TG_sCommandRef.s32SetpointValue;
				if (UG_sSystemControllers.sCommandAll.s32IqCommand > _IQ(0.2))
					UG_sSystemControllers.sCommandAll.s32IqCommand = _IQ(0.2);
				break;
			case UGT_E_CM_SPEED:
				UG_sSystemControllers.sCommandAll.s32SpdCommand = TG_sCommandRef.s32SetpointValue ;//+ MDS_sCP.PI_CP.s32Out;
				MDS_sCP.PI_CP.s32Ref = MDS_sCP.s32PowerMax; // 限制速度环下最大输出功率
	...
	if (UG_sSystemControllers.sSPD.u16LoopIndex == 0)
		UG_sSystemControllers.sSPD.u16LoopIndex = UPDS_SPD_LOOP_COUNT;
