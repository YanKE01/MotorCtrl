...
void MDS_vMotorControlState(void)
{
	...
	{
	case UGT_E_SS_IDLE_A:
		while (MDS_vIdle() == 0)
			;
		break;
	case UGT_E_SS_INIT_B:
		while (MDS_vInit() == 0)
			;
		break;
	case UGT_E_SS_STOP_C:
		break;
	case UGT_E_SS_START_D:
		while (MDS_vStart() == 0)
			;
		break;
	case UGT_E_SS_PRECHARGE_E:
		break;
	case UGT_E_SS_TRACK_F:
		break;
	case UGT_E_SS_IDENTIFY_G:
		break;
	case UGT_E_SS_ALIGNMENT_H:
		break;
	case UGT_E_SS_OPENLOOP_I:
		break;
	case UGT_E_SS_CLOSEDLOOP_J:
		break;
	case UGT_E_SS_BRAKE_K:
		break;
	case UGT_E_SS_FAULT_L:
		break;
	case UGT_E_SS_WAIT_M:
		break;
	case UGT_E_SS_WRITE_FLASH:
		while (MDS_vWriteFlash() == 0)
			;
		break;
	default:
		break;
...
u8 MDS_vIdentify(void)
{
	...
	if (UG_sSysStateErr.u8IdentifySwitch == 1)
	{
		...
		if (UG_sSystemControllers.u8ExtraStart == 1)
			UG_sSysStateErr.uControlReg.Bits.ControlMode = UGT_E_CM_IF;
...
u8 MDS_vOpenloop(void)
{
	...
	if (s16PosErr < -MDS_THETSAERR_160DEG)
		s16PosErr += 0xFFFF;
	else if (s16PosErr > MDS_THETSAERR_160DEG)
		s16PosErr -= 0xFFFF;
...
u8 MDS_vClosedloop(void)
{
	...
	if (UG_sSysStateErr.sSoftClose.u8SoftCloseEn == FALSE)
	{
		...
		if (UG_sSysStateErr.sPWMMode.PWMModePre != UG_sSysStateErr.sPWMMode.PWMMode)
		{
			...
			if (UG_sSystemControllers.sSPD.u16LoopIndex == UPDS_SPD_LOOP_COUNT)
				UG_sSystemControllers.sSPD.u16LoopIndex -= 2;
		}
	...
	else
	{
		if (UG_sSysStateErr.sSoftClose.u8SoftCloseLastEn == FALSE)
		{
			...
			if (UG_sSystemControllers.sCommandAll.s32IqCommand < _IQ(0.0))
				UG_sSystemControllers.sCommandAll.s32IqCommand = _IQ(0.0);
		}
		else
		{
			UG_sSystemControllers.sCommandAll.s32IdCommand -= _IQ(0.0001);
			if (UG_sSystemControllers.sCommandAll.s32IdCommand < _IQ(0.0))
				UG_sSystemControllers.sCommandAll.s32IdCommand = _IQ(0.0);
			UG_sSystemControllers.sCommandAll.s32IqCommand -= UPDS_SOFTCLOSE_IQ_DEACC_MS_IQ;
			if (UG_sSystemControllers.sCommandAll.s32IqCommand < _IQ(0.0))
				UG_sSystemControllers.sCommandAll.s32IqCommand = _IQ(0.0);
		}
...
u8 MDS_vFault(void)
{
	if (UG_sSysStateErr.sSystemState > UGT_E_SS_INIT_B)
		US_vSysFaultProcess();
	return 1;
}
...
void MDS_vMotorAppCommand(void)
{
	...
	case UPDS_COMMAND_KEIL:
	{
		...
		UG_sSystemControllers.s32Command = HY_SetPumpMechanicalSpeed(motor.targetSpeed); // 调速控制接口
		if (motor.state == 0)
		{
			UG_sSysStateErr.sSoftClose.s16SystemSwitch = 0; // 0;//starup
		}
		else
		{
...
void MDS_vFGRPM(void)
{
	...
	else
	{
		if ((UG_sPositionAll.PosCount[0] < 0x7FFF) && (UG_sPositionAll.PosCount[1] >= 0x7FFF))
		{
			...
			if (UG_sPositionAll.PosCountIndex == (UPDS_PAIRS + 1))
				UG_sPositionAll.PosCountIndex = 1;
		}
		else if ((UG_sPositionAll.PosCount[0] >= 0x7FFF) && (UG_sPositionAll.PosCount[1] < 0x7FFF))
		{
			UG_sPositionAll.PosCountIndex--;
			if (UG_sPositionAll.PosCountIndex == 0)
				UG_sPositionAll.PosCountIndex = 1;
		}
	}
	...
	else if (UG_sPositionAll.PosCountIndex == UPDS_PAIRS)
