...
static void tim15_cfg_init()
{
	...
	TIM_TimeBaseInit(TIM15, &TIM_TimeBaseStructure);			//初始化定时器时基
	...
	TIM_ICInitStructure.TIM_Channel = TIM_Channel_2;				// TIM15 CH2 配置为输入捕获模式
	...
	TIM_ICInit(TIM15, &TIM_ICInitStructure);						//初始化TIM15 CH2

	TIM_SelectOnePulseMode(TIM15, TIM_OPMode_Single); //选择定时器为单脉冲模式
	TIM_DTS_Cfg(TIM15);
	TIM_TI2E_Set(TIM15);
	TIM_SelectInputTrigger(TIM15, TIM_TS_TI2FP2);	   //选择触发源为IC2
	TIM_SelectSlaveMode(TIM15, TIM_SlaveMode_Trigger); //选择定时器从模式为上升沿触发
