...
static void tim17_cfg_init()
{
	...
	TIM_TimeBaseInit(TIM17, &TIM_TimeBaseStructure);			//初始化定时器时基
	...
	TIM_SelectOnePulseMode(TIM17, TIM_OPMode_Single); //选择定时器为单脉冲模式
...
int main()
{
	...
	{
		delay(100);
		TIM_Cmd(TIM17, ENABLE); //产生一个周期的PWM后计数停止，需要手动打开
	}
}
