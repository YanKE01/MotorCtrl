...
void DAC0_cfg()
{
  ...
  DAC_Cmd(DAC0, ENABLE);
  DAC_SetDac_10B_Data(DAC0, DAC_Align_10B_R, 0x136);
...
void acmp2_cfg()
{
  ...
  GPIO_PinAFConfig(GPIOB, GPIO_PinSource1, GPIO_AF_1); // CPP20
  ...
  ACMP_InitStruct.ACMP_FS = ENABLE;                  //输出滤波使能配置
  ...
  ACMP2_SEQ_INPUT_SELECT(DISABLE, 0, 1); //选择输入通道：是否连续；正端；负端
  ACMP2_Cmd(ENABLE);
}
...
void TIM3_CFG_CH1()
{
  ...
  TIM3_IntputSourceCfg(TIM3_INPUT_TI1, TIM3_IntputSource_ACMP2_OUTPUT); // 配置捕获源：TIM3 TI1捕获ACMP2_OUTPUT
...
int main()
{
  ...
  acmp2_cfg(); // PA5(OUT) PB1(P) DAC1(N)

  TIM3_CFG_CH1(); // TIM1 SET
  while (1)
    ;
}
