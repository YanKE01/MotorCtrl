...
extern "C"
{
  ...
  typedef struct
  {
    ...
    uint32_t ADC_ClkMode; /*   ADC 时钟分频                                       */

    uint32_t ADC_Resolution; /*  adc采样数据位宽设置 此型号暂时只支持12bit           */

    FunctionalState ADC_ContinuousConvMode; /*  连续转换模式是否使能                                */

    FunctionalState ADC_AutoWatiMode; /*自动延迟转换，一般保证多通道数据顺序不错乱            */

    FunctionalState ADC_DMA; /* DMA*/

    FunctionalState ADC_DiscMode; /* 断续模式*/

    FunctionalState ADC_SamecMode; /*同时采样 */
    ...
    uint32_t ADC_DMATRIG_LEVEL; /*置触发 DMA 请求的 FIFO 水平。在 DMAEN 使能前进行配置*/
...
/*--------------------------------------------------------------------------------------------
 ...
 Going out of range can lead to unpredictable errors;
  --------------------------------------------------------------------------------------------*/
//同步时钟 sysclk
#define ADC_ClockMode_sysnClk ((uint32_t)0x00000000) /*!< 与系统时钟同步 */
...
//异步时钟pllclk
#define ADC_ClockMode_PLLClk ((uint32_t)0x80000000) /*!< 从pll时钟来，属于异步时钟 */
...
#define IS_ADC_AnalogWatchdog_Channel(ADC_AnalogWatchdog_Channel) ((ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_0) ||  \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_1) ||  \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_2) ||  \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_3) ||  \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_4) ||  \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_5) ||  \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_6) ||  \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_7) ||  \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_8) ||  \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_9) ||  \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_10) || \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_11) || \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_12) || \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_13) || \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_14) || \
                                                                   (ADC_AnalogWatchdog_Channel == ADC_AnalogWatchdog_Channel_15))
...
#define ADC_TESTCFG_TEST_Msk (0x1UL)      /*!< TEST (Bitfield-Mask: 0x01)                            */
  ...
  void ADC_Init(ADC_TypeDef *ADCx, ADC_InitTypeDef *ADC_InitStruct); //初始化adc 共有12个参数需要配置
  void ADC_StructInit(ADC_InitTypeDef *ADC_InitStruct);              // ADC 参数结构体初值定义
  void ADC_Cmd(ADC_TypeDef *ADCx, FunctionalState NewState);         //使能ADC

  /* Power saving functions *****************************************************/
  void ADC_AutoPowerOffCmd(ADC_TypeDef *ADCx, FunctionalState NewState); // adc 自动掉电使能函数

  /* Analog Watchdog configuration functions ************************************/
  void ADC_AnalogWatchdogCmd(ADC_TypeDef *ADCx, FunctionalState NewState);                                   // ADC模拟开门狗
  void ADC_AnalogWatchdogThresholdsConfig(ADC_TypeDef *ADCx, uint16_t HighThreshold, uint16_t LowThreshold); //模拟看门狗限值
  void ADC_AnalogWatchdogSingleChannelConfig(ADC_TypeDef *ADCx, uint32_t ADC_AnalogWatchdog_Channel);        //单通道看门狗选择
  void ADC_AnalogWatchdogSingleChannelCmd(ADC_TypeDef *ADCx, FunctionalState NewState);                      //单通道看门狗使能

  /* Temperature Sensor , Vrefint and Vbat management function ******************/
  void ADC_TempSensorCmd(FunctionalState NewState); //温度传感器采样使能
  void ADC_VrefintCmd(FunctionalState NewState);    //内部电压采样使能
  ...
  void ADC_StopOfConversion(ADC_TypeDef *ADCx);       //停止ADC转换
  void ADC_StartOfConversion(ADC_TypeDef *ADCx);      //开始adc转换
  uint16_t ADC_GetConversionValue(ADC_TypeDef *ADCx); //读取转换结果
  void ADC_CloseAdc(ADC_TypeDef *ADCx);               //关闭ADC模块

  void ADC_VrefintSelect(ADC_TypeDef *ADCx, uint32_t ADC_Vref); // ADC参考电压选择

  /* Interrupts and flags management functions **********************************/
  void ADC_ITConfig(ADC_TypeDef *ADCx, uint32_t ADC_IT, FunctionalState NewState); // ADC中断使能
  FlagStatus ADC_GetFlagStatus(ADC_TypeDef *ADCx, uint32_t ADC_FLAG);              // ADC状态标志位读取
  void ADC_ClearFlag(ADC_TypeDef *ADCx, uint32_t ADC_FLAG);                        //清ADC状态标志位
  ITStatus ADC_GetITStatus(ADC_TypeDef *ADCx, uint32_t ADC_IT);                    //获取中断标志位
  void ADC_ClearITPendingBit(ADC_TypeDef *ADCx, uint32_t ADC_IT);                  //清除中断标志位
